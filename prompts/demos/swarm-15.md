...
EA first registers a project, then spawns only the root. Paste the root task below and use the `spawn_agent` MCP tool:

```
add_project({"name": "swarm-15-chat-system"})
# → {"project_id": 1, ...}
...
The root spawns the rest of the tree via its own `spawn_agent` calls, reusing the same `project_id`. EA monitors `claude-architect` for `[TASK COMPLETE]` but does NOT kill any agents.
...
- Use `schedule_omar_event` for check-ins; never sleep loops.
...
- After `[TASK COMPLETE]`, always wake your parent immediately by calling `schedule_omar_event({"receiver": "<parent_name>", "payload": "[CHILD COMPLETE] YOUR_NAME: <summary>", "delay_seconds": 0})`.
...
4. Wait for `[CHILD COMPLETE]` messages from both. Use `get_agent` + `schedule_omar_event` as fallback if a child hasn't notified after a reasonable time.
5. Run the tests: `cd junk/chat-system && npm install && npm test`.
6. Report [TASK COMPLETE] with results. Wake the EA: `schedule_omar_event({"receiver": "ea", "payload": "[CHILD COMPLETE] claude-architect: <results>", "delay_seconds": 0})`. Do NOT kill any agents.
```
...
3. Wait for `[CHILD COMPLETE]` from both. Use `get_agent` + `schedule_omar_event` as fallback.
4. Write junk/chat-system/src/server.js: wire api.js, ws.js, db.js, and auth.js into an Express+WebSocket server on port 3000.
5. Report [TASK COMPLETE]. Wake the parent: `schedule_omar_event({"receiver": "claude-architect", "payload": "[CHILD COMPLETE] codex-backend: Backend complete.", "delay_seconds": 0})`. Do NOT kill any agents.
```
...
3. Wait for `[CHILD COMPLETE]` from both. Use `get_agent` + `schedule_omar_event` as fallback.
4. Report [TASK COMPLETE]. Wake the parent: `schedule_omar_event({"receiver": "claude-architect", "payload": "[CHILD COMPLETE] opencode-frontend: Frontend complete.", "delay_seconds": 0})`. Do NOT kill any agents.
```
...
3. Wait for `[CHILD COMPLETE]` from both. Use `get_agent` + `schedule_omar_event` as fallback. Verify src/api.js and src/ws.js exist.
4. Report [TASK COMPLETE]. Wake the parent: `schedule_omar_event({"receiver": "codex-backend", "payload": "[CHILD COMPLETE] cursor-api: API layer complete.", "delay_seconds": 0})`. Do NOT kill any agents.
```
...
3. Wait for `[CHILD COMPLETE]` from both. Use `get_agent` + `schedule_omar_event` as fallback. Verify src/db.js and src/auth.js exist.
4. Report [TASK COMPLETE]. Wake the parent: `schedule_omar_event({"receiver": "codex-backend", "payload": "[CHILD COMPLETE] claude-data: Data layer complete.", "delay_seconds": 0})`. Do NOT kill any agents.
```
...
3. Wait for `[CHILD COMPLETE]` from both. Use `get_agent` + `schedule_omar_event` as fallback. Verify their files exist.
4. Report [TASK COMPLETE]. Wake the parent: `schedule_omar_event({"receiver": "opencode-frontend", "payload": "[CHILD COMPLETE] codex-ui: UI complete.", "delay_seconds": 0})`. Do NOT kill any agents.
```
...
3. Wait for `[CHILD COMPLETE]` from both. Use `get_agent` + `schedule_omar_event` as fallback. Verify their files exist.
4. Report [TASK COMPLETE]. Wake the parent: `schedule_omar_event({"receiver": "opencode-frontend", "payload": "[CHILD COMPLETE] opencode-infra: Infra complete.", "delay_seconds": 0})`. Do NOT kill any agents.
```
...
Report [TASK COMPLETE], then wake the parent: `schedule_omar_event({"receiver": "cursor-api", "payload": "[CHILD COMPLETE] claude-rest: REST API complete: src/api.js written.", "delay_seconds": 0})`.
```
...
Report [TASK COMPLETE], then wake the parent: `schedule_omar_event({"receiver": "cursor-api", "payload": "[CHILD COMPLETE] codex-ws: WebSocket complete: src/ws.js written.", "delay_seconds": 0})`.
```
...
Export all. Report [TASK COMPLETE], then wake the parent: `schedule_omar_event({"receiver": "claude-data", "payload": "[CHILD COMPLETE] opencode-db: Storage complete: src/db.js written.", "delay_seconds": 0})`.
```
...
Export all. Report [TASK COMPLETE], then wake the parent: `schedule_omar_event({"receiver": "claude-data", "payload": "[CHILD COMPLETE] cursor-auth: Auth complete: src/auth.js written.", "delay_seconds": 0})`.
```
...
Report [TASK COMPLETE], then wake the parent: `schedule_omar_event({"receiver": "codex-ui", "payload": "[CHILD COMPLETE] claude-chat: Chat UI complete: public/index.html and public/app.js written.", "delay_seconds": 0})`.
```
...
Report [TASK COMPLETE], then wake the parent: `schedule_omar_event({"receiver": "codex-ui", "payload": "[CHILD COMPLETE] codex-admin: Admin panel complete: public/admin.html and public/admin.js written.", "delay_seconds": 0})`.
```
...
Report [TASK COMPLETE], then wake the parent: `schedule_omar_event({"receiver": "opencode-infra", "payload": "[CHILD COMPLETE] opencode-cli: CLI client complete: src/cli.js written.", "delay_seconds": 0})`.
```
...
Report [TASK COMPLETE], then wake the parent: `schedule_omar_event({"receiver": "opencode-infra", "payload": "[CHILD COMPLETE] cursor-tests: Test suite complete: test/ directory written with 20+ cases.", "delay_seconds": 0})`.
```
...
- If a worker stalls, the lead above should notice; do not kill it in this demo. Instead, send a nudge via `schedule_omar_event`.
