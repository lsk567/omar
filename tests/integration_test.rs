...
/// Test that delivering an event to the EA manager pane does not corrupt
/// in-progress (partial) input the user may be composing.
///
/// Bug D: `deliver_to_tmux` used to inject the event text directly into
/// the pane's input buffer via `send-keys -l`.  If the user had already
/// typed a partial multi-line message (using shift+enter), the event text
/// would be appended to that in-progress input, corrupting or truncating
/// it before the user pressed Enter.
///
/// Fix: for the manager pane only, send Enter first to flush any partial
/// input, then deliver the event.  The test simulates this by sending
/// partial text, then replicating the fixed delivery sequence, and
/// asserting that (a) the event payload arrives intact and (b) the partial
/// text was submitted as its own line (not merged with the event).
#[test]
fn test_manager_pane_event_does_not_corrupt_partial_input() {
    if !tmux_available() {
        eprintln!("Skipping test: tmux not available");
        return;
    }
    ...
    const BASE_PREFIX: &str = "omar-agent-";
    // Manager session: ea_manager_session(0, BASE_PREFIX) = "omar-agent-ea-0"
    let manager_session = format!("{}ea-0", BASE_PREFIX);

    // Pre-cleanup
    let _ = tmux(&["kill-session", "-t", &manager_session]);

    // Create a session that behaves like a shell (simulates the manager pane)
    let r = tmux(&["new-session", "-d", "-s", &manager_session]);
    assert!(
        r.is_ok(),
        "Failed to create manager session '{}': {:?}",
        manager_session,
        r
    );
    ...
    // Simulate the user having typed a partial message (not yet submitted)
    // using send-keys -l (literal, no Enter).
    let partial_input = "partial-user-input-in-progress";
    let _ = tmux(&["send-keys", "-t", &manager_session, "-l", partial_input]);

    // Simulate the fixed deliver_to_tmux sequence for a manager pane:
    //   1. Send Enter to flush any partial input already in the buffer
    //   2. Send the event text (literal)
    //   3. Send Enter to submit the event
    let event_payload = "EVENT_PAYLOAD_MANAGER_PANE";

    // Step 1: flush partial input
    let _ = tmux(&["send-keys", "-t", &manager_session, "Enter"]);
    // Step 2+3: deliver event
    let _ = tmux(&["send-keys", "-t", &manager_session, "-l", event_payload]);
    let _ = tmux(&["send-keys", "-t", &manager_session, "Enter"]);

    thread::sleep(Duration::from_millis(500));

    // Capture pane output
    let out = tmux(&["capture-pane", "-t", &manager_session, "-p"]).unwrap_or_default();

    // The event payload must arrive intact (not merged with partial_input)
    assert!(
        out.contains(event_payload),
        "Event payload '{}' must appear in manager pane: {}",
        event_payload,
        out
    );

    // The partial input must appear on its own line — not concatenated with
    // the event payload on the same line.
    let merged = format!("{}{}", partial_input, event_payload);
    assert!(
        !out.contains(&merged),
        "Partial input and event payload must NOT be on the same line (BUG D): {}",
        out
    );
