...
pub fn run_manager_orchestration(
    ...
    loop {
        ...
        match input {
            ...
            "approve" | "y" => {
                approve_plan(client, command, &session, ea_id, omar_dir)?;
            }
            "reject" | "n" => {
...
fn approve_plan(
    ...
    }) = parse_manager_message(&output)
    {
        ...
        for agent in &agents {
            spawn_worker(client, agent, command, ea_id, omar_dir)?;
        }
...
fn reject_plan(client: &TmuxClient, session: &str) -> Result<()> {
    ...
    send_to_manager(client, session, &format!("Plan rejected. Reason: {}", reason.trim()))?;
