        ...
        Some(Commands::Manager { action }) => {
            ...
            match action {
                Some(ManagerAction::Start) | None => {
                    manager::start_manager(&ea_client, &config.agent.default_command, ea_id, "Default", &omar_dir, &config.dashboard.session_prefix)
                }
                Some(ManagerAction::Orchestrate) => {
                    manager::run_manager_orchestration(&ea_client, &config.agent.default_command, ea_id, "Default", &omar_dir, &config.dashboard.session_prefix)
                }
            }
        }
        None => {
...
async fn run_dashboard(config: Config) -> Result<()> {
    ...
    if tmux_setup_needed() {
        shared_app.lock().await.set_persistent_warning("⚠ tmux not configured for omar — run 'omar setup-tmux' to fix");
    }
    ...
    loop {
            ...
            match event {
                AppEvent::Key(key) => {
                    ...
                    if app.ea_input_mode {
                        match key.code {
                            ...
                            KeyCode::Enter => {
                                ...
                                if !name.trim().is_empty() {
                                    app.create_ea(name.trim().to_string(), None);
                                }
                                app.ea_input_mode = false;
                                app.ea_input.clear();
                            }
                    ...
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            ...
                            *popup_receiver.lock().unwrap() = app
                            ...
                            if std::env::var("TMUX").is_ok() {
                                ...
                                let popup_info = {
                                    let app = shared_app.lock().await;
                                    app.selected_agent().map(|a| {
                                        (a.session.name.clone(), app.client().clone())
                                    })
                                }; // Lock released here
                        ...
                        KeyCode::Char('D') => {
                            // Delete the currently active EA (EA 0 is protected)
                            if app.active_ea != 0 {
                                app.pending_confirm = Some(app::ConfirmAction::DeleteEa);
                            } else {
                                app.set_status("Cannot delete EA 0");
                            }
                        }
                        KeyCode::Char('p') => {
        ...
        // Check quit flag
        if shared_app.lock().await.should_quit {
            break;
        }
    }
